...
def run_backtest():
    ...
    test_df = df[df['date'].dt.year == 2024].copy() # Use .copy() to avoid SettingWithCopyWarning
    ...
    print("--- Evaluating Return Prediction Model ---")
    ...
    if os.path.exists(return_model_path):
        ...
        print(classification_report(y_test_return, predictions))
        print("Confusion Matrix (Returns):")
        print(confusion_matrix(y_test_return, predictions))
        print(f"Accuracy (Returns): {accuracy_score(y_test_return, predictions):.4f}")
    ...
    if os.path.exists(volatility_model_path):
        ...
        print(classification_report(y_test_vol, predictions))
        print("Confusion Matrix (Volatility):")
        print(confusion_matrix(y_test_vol, predictions))
        print(f"Accuracy (Volatility): {accuracy_score(y_test_vol, predictions):.4f}")
